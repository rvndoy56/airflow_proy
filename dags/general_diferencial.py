...
def extract_transform_load(table_name, attributes, id_column):
    @task
    def extract_data_from_mysql():
        mysql_hook = MySqlHook(mysql_conn_id='mysql_conn_id')
        last_processed_id = Variable.get(f"{table_name}_last_processed_id", default_var=0)
        query = f"SELECT * FROM `bd-grupo-3`.{table_name} WHERE {id_column} > {last_processed_id}"
    ...
    def update_last_processed_id():
        if data:
            new_last_processed_id = max(row[id_column] for row in data)
            Variable.set(f"{table_name}_last_processed_id", new_last_processed_id)
...
# Define the DAG
@dag(dag_id="dag_general_diferencial",default_args=default_args, schedule_interval=None, start_date=days_ago(1), catchup=False, tags=['mysql_example_general'])
def mysql_example_dag():
    ...
    for table_name, attributes in tables.items():
        extract_transform_load(table_name, attributes, id_columns[table_name])
