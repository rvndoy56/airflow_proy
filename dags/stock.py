...
S3_OBJECT_NAME="landing/customers/stock.csv"
...
def mysql_example_dag():
    ...
    def extract_data_from_mysql():
        ...
        query = "select * from `bd-grupo-3`.Stock"
    ...
    def create_csv(transformed_data):
        try:
            ...
            with open(CSV_FILE_PATH, 'w') as file:
                file.write("id_stock,id_producto,stock_actual,stock_valorizado,fecha_actualizacion\n")
                for row in rows:
                    file.write(f"{row['id_stock']},{row['id_producto']},{row['stock_actual']},{row['stock_valorizado']},{row['fecha_actualizacion']}\n")
